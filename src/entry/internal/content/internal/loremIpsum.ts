...
// TODO: If `cheerio` works better
// TODO: (see `wrapEntryLeads`),
// TODO: replace `JSDOM` here with `cheerio`.
...
export function replaceHtmlTextWithLoremIpsum(htmlText: string) {
    ...
    function replaceTextContent(node: Node) {
        if (node.nodeType === dom.window.Node.TEXT_NODE) {
            ...
            if (len > 0 && text.trim()) {
                const localLoremIpsum =
                    LOREM_IPSUM.substring(loremIpsumPosition) +
                    LOREM_IPSUM.substring(0, loremIpsumPosition);
                node.nodeValue = localLoremIpsum
                    .substring(0, text.length)
                    .trim();
                node.nodeValue =
                    node.nodeValue.charAt(0).toUpperCase() +
                    node.nodeValue.slice(1);
                loremIpsumPosition =
                    (loremIpsumPosition + len) % LOREM_IPSUM.length;
            }
        } else {
